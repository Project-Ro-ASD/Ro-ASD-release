#!/usr/bin/env python3
"""Create a CI-only build manifest from locally built RPM files.

This manifest describes a build result. It is deliberately NOT the Ro-Repo
component-artifact-manifest-v1 release contract because a CI build has no
immutable GitHub Release ID yet.
"""
...
def main() -> int:
    ...
    if not artifacts:
        raise ValueError("build manifest için RPM/SRPM bulunamadı")
