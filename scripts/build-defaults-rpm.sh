...
pkg_dir="${repo_root}/packages/ro-asd-defaults"
version_file="${pkg_dir}/VERSION.yaml"
dist_dir="${repo_root}/dist"
mkdir -p "${repo_root}/build"
work_dir="$(mktemp -d "${repo_root}/build/defaults.XXXXXX")"
trap 'rm -rf -- "${work_dir}"' EXIT

python3 "${repo_root}/scripts/check-defaults-policy.py"

component_version="$(awk '$1 == "component_version:" { print $2 }' "${version_file}")"
source_name="ro-asd-defaults-${component_version}"
...
install -m 0644 "${repo_root}/docs/DEFAULTS-OWNERSHIP-V1.md" "${source_dir}/DEFAULTS-OWNERSHIP-V1.md"
install -m 0644 "${pkg_dir}/defaults-policy-v1.json" "${source_dir}/defaults-policy-v1.json"

tar --sort=name --mtime='@0' --owner=0 --group=0 --numeric-owner \
  -C "${work_dir}" -czf "${topdir}/SOURCES/${source_name}.tar.gz" "${source_name}"
install -m 0644 "${pkg_dir}/ro-asd-defaults.spec" "${topdir}/SPECS/ro-asd-defaults.spec"

rpmbuild -ba \
  --define "_topdir ${topdir}" \
  --define "_tmppath ${work_dir}/tmp" \
  --define "dist .fc44" \
  "${topdir}/SPECS/ro-asd-defaults.spec"
