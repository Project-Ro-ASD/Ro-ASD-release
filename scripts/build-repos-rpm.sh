...
version_file="${repo_root}/packages/ro-asd-repos/VERSION.yaml"
spec_file="${repo_root}/packages/ro-asd-repos/ro-asd-repos.spec"
dist_dir="${repo_root}/dist"
mkdir -p "${repo_root}/build"
work_dir="$(mktemp -d "${repo_root}/build/repos.XXXXXX")"
trap 'rm -rf -- "${work_dir}"' EXIT

python3 "${repo_root}/scripts/check-repos-version.py"

component_version="$(awk '$1 == "component_version:" { print $2 }' "${version_file}")"
source_name="ro-asd-repos-${component_version}"
...
install -m 0644 "${repo_root}/packages/ro-asd-repos/ro-asd.repo" "${source_dir}/ro-asd.repo"

tar --sort=name --mtime='@0' --owner=0 --group=0 --numeric-owner \
  -C "${work_dir}" -czf "${topdir}/SOURCES/${source_name}.tar.gz" "${source_name}"
install -m 0644 "${spec_file}" "${topdir}/SPECS/ro-asd-repos.spec"

rpmbuild -ba \
  --define "_topdir ${topdir}" \
  --define "_tmppath ${work_dir}/tmp" \
  --define "dist .fc44" \
  "${topdir}/SPECS/ro-asd-repos.spec"
